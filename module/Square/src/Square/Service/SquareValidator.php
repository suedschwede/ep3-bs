    ...
    public function isBookable($dateStart, $dateEnd, $timeStart, $timeEnd, $square)
    {
        ...
        if ($user) {
            ...
            if ($maxActiveBookings != 0) {
                ...
                $shortTermBookingEnd->modify('+3 hours');
                ...
                if ($bookable) {
                    ...
                    if ($dateStart < $shortTermBookingStart || $dateStart > $shortTermBookingEnd)
                    {
                        if ($hoursBetweenBookings && $minDiffHours < $hoursBetweenBookings) {
                            $bookable = false;
                            $notBookableReason = 'Es m�ssen mindestens ' . $hoursBetweenBookings . ' Stunden zwischen 2 Buchnungen liegen.';
                        }
                    }
                    ...
                    if ($bookable && $activeBookingsCountWithoutShort >= $maxActiveBookings) {
                        ...
                        if (!$bookable) {
                            $notBookableReason = 'Sie k�nnen derzeit nur <b>' . $maxActiveBookings . ' aktive Buchung(en)</b> plus eine Spontanbuchung gleichzeitig offen haben.' . "<br>"; // . $shortTermBookingStart->format(DateTime::ISO8601) . "<br>" . $shortTermBookingEnd->format(DateTime::ISO8601) . "<br>" . $dateStart->format(DateTime::ISO8601);
                        }
                    }
                }
            }
